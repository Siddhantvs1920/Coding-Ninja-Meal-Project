    ...
    meals.then(data=>{
        if (data.meals) {
            data.meals.forEach((element) => {
                ...
                if (isFav) {
                    html += `
                    <div id="card" class="card mb-3" style="width: 17rem;">
                    <img src="${element.strMealThumb}" class="card-img-top" alt="...">
                    <div class="card-body">
                    <h4 class="card-title">${element.strMeal}</h4>
                    <div class="d-flex justify-content-between mt-5">
                    <button type="button" class="btn btn-outline-info" onclick="showMealDetails(${element.idMeal})">More Details</button>
            ...
            Sorry for the incovience!! Serch Different Meal.
...
// Show Meal Details when user click on more details
async function showMealDetails(id) {
    let url = "https://www.themealdb.com/api/json/v1/1/lookup.php?i=";
    let html = "";
    await fetchMealsFromApi(url, id).then(data => {
        const instructionsArray = splitInstructions(data.meals[0].strInstructions);

        html += `
        <div id="meal-details" class="mb-5">
        <a id="back-button" href="#" onclick="closeMealDetails()">Back</a>
...
//To Split the instruction in to multiple line based on some condition
function splitInstructions(instruction) {
    const regex = /\bSTEP \d+\b/g;
    const stepsArray = instruction.match(regex) || [];
    const instructionsArray = [];

    for (let i = 0; i < stepsArray.length; i++) {
        const currentStep = stepsArray[i];
        const currentIndex = instruction.indexOf(currentStep);
        const nextIndex = i < stepsArray.length - 1 ? instruction.indexOf(stepsArray[i + 1]) : instruction.length;

        const currentInstruction = instruction.substring(currentIndex, nextIndex).trim();
        instructionsArray.push(currentInstruction);
    }
...
async function showFavMealList() {
    ...
    let url="https://www.themealdb.com/api/json/v1/1/lookup.php?i=";
    let html="";
    //if no meal is added in favourite list then 
    if (arr.length==0) {
        html += `
        <div class="page-wrap d-flex flex-row align-items-center">
        <div class="container">
        ...
        for (let index = 0; index < arr.length; index++) {
            await fetchMealsFromApi(url,arr[index]).then(data=>{
                ...
                <div class="d-flex justify-content-between mt-5">
                <button type="button" class="btn btn-outline-light btn-outline-info" onclick="showMealDetails(${data.meals[0].idMeal})">More Details</button>
